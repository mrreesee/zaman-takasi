...
// Başlangıç (host kurulmadan önceki) hataları da yakalansın diye bootstrap logger.
...
try
{
    ...
    // Serilog: Console + günlük rolling file. Minimum seviye appsettings "Serilog" bölümünden.
    builder.Host.UseSerilog((context, services, configuration) => configuration
        .ReadFrom.Configuration(context.Configuration)
        .ReadFrom.Services(services)
        .Enrich.FromLogContext()
        .WriteTo.Console()
        .WriteTo.File("logs/zamantakasi-.log", rollingInterval: RollingInterval.Day, retainedFileCountLimit: 14));

    // Bağlantı cümlesi env'den gelir (sağlayıcı bağımsızlığı). Öncelik: DATABASE_URL (Railway/Neon URI'si),
    // sonra ConnectionStrings:Default (env veya lokal user-secrets/appsettings.Development). URI ise key-value'ya çevrilir.
    var rawConn = Environment.GetEnvironmentVariable("DATABASE_URL")
                  ?? builder.Configuration.GetConnectionString("Default");
    var connectionString = NpgsqlConnectionResolver.Resolve(rawConn)
        ?? throw new InvalidOperationException(
            "Veritabanı bağlantısı tanımlı değil. 'DATABASE_URL' ortam değişkenini ya da 'ConnectionStrings:Default' değerini ayarlayın.");
    ...
    // HTTP isteklerini yapılandırılmış (structured) olarak logla.
